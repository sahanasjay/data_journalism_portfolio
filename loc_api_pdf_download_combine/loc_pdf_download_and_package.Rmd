...
## Use Library of Congress API to retrieve list of newspapers that are available in Chronicling America
  # Store path to all newspapers -- not just CA titles -- API
  ...
  # Select only the content from the respons and convert to characters
...
```{r}
# Create a list of issue urls by calling the issue_url column and converting to a vector. This is the list that we will loop through to retrieve page urls for each newspaper issue.
...
for(folder in current_title_level_dirs){
  ...
  for(dir in victim_level_dirs){
    ...
    for(i in historic_title_level_dirs){
      ...
      # Appending rows (for historical titles associated with a single victim) into "empty" df  
      ...
      for(issue in ls_issue_folders_list){
        ...
        #### NOTE: Majority of the else statements here are placeholder text for the ongoing portion of this project. They will be updated when the project is published. 
        else{
    ...
    pdf_combine(list.files(path=i, pattern="bound", full.names=TRUE, recursive = TRUE), output = paste0(i,"/", "bound_historical_packet.pdf"))
    ...
    dev.off()
    # Creating victim pdf
    pdf_combine(list.files(path=dir, pattern = "packet", full.names=TRUE, recursive = TRUE), output = paste0(dir, "/", "victim_level_bound_packet.pdf"))
